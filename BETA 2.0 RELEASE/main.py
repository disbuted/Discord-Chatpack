...
# [=] Attempting to add some sort of junked up discord.id "api" for retrieving the user info via their id from ,info <USERID>
...
async def on_message(message):
    ...
    if message.content == ",cmds":
        ...
        random_list = [
            ...
            ",user <userid> : Shows info of the userId sent",
    ...
    if message.content.startswith(",delete"):
      ...
      #  last_confirmation = await message.channel.send(f"Deleted {deleted} messages.")
        ...
        parts = message.content.split()
        if len(parts) < 2:
            await message.channel.send("Please provide a user ID. Usage: `,info <user_id>`")
            return

        user_id = parts[1]  

        try:
            url = f"https://discord.id/?id={user_id}"
            headers = {
            ...
            if response.status_code != 200:
                await message.channel.send(f"Failed to fetch user info. HTTP Status: {response.status_code}")
                return
            ...
            username_element = soup.find("td", text="Username")
            created_element = soup.find("td", text="Created")
            avatar_element = soup.find("img", class_="avatar")

            if not username_element or not created_element or not avatar_element:
                await message.channel.send("Could not extract user info. The website structure may have changed.")
                return

            username = username_element.find_next_sibling("td").text.strip()
            created_at = created_element.find_next_sibling("td").text.strip()
            ...
            embed.add_field(name="Profile Link", value=f"[View Profile](https://discord.id/?id={user_id})", inline=False)

            await message.channel.send(embed=embed)

        except Exception as e:
            await message.channel.send(f"Error fetching user info: {str(e)}")
